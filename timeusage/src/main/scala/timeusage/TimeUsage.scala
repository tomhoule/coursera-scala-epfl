...
object TimeUsage {
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = expr("CASE WHEN teage > 55 THEN 'elder' WHEN teage > 22 THEN 'active' ELSE 'young' END")
      .as("age")

    val primaryNeeds = primaryNeedsColumns.mkString(" + ")
    val primaryNeedsProjection: Column = expr(s"($primaryNeeds) / 60").as("primaryNeeds")
    val work = workColumns.mkString(" + ")
    val workProjection: Column = expr(s"($work) / 60").as("work")
    val other = otherColumns.mkString(" + ")
