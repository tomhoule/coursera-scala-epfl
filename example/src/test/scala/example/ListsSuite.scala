 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of list with repeated elements") {
    assert(sum(List(9, 3, 3)) === 15)
  }

  test("sum with negative numbers can be below 0") {
    assert(sum(List(-2, 33, -55, -12)) === -36)
  }
  ...
  test("max of negative numbers") {
    assert(max(List(-25, -2, -7, -33)) === -2)
  }

  test("max of list of length 1 is head") {
    assert(max(List(0)) === 0)
  }

  test("max of empty list fails") {
